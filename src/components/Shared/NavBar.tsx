        ...
        <div className="flex items-center justify-between">
          <Link href="/" className="cursor-pointer -mt-1.5">
            <div className="flex items-center justify-center gap-1">
              ...
              <div>
                <h1 className="font-bold text-xl tracking-tight text-green-900 mt-1">Human Care</h1>
              </div>
            </div>
          </Link>
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) =>
              link.isRoute ? (
                <Link key={link.href} href={link.href} className="text-slate-800 hover:text-green-600 transition-colors relative group font-medium">
                  {link.label}
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#10B981] group-hover:w-full transition-all"></span>
                </Link>
              ) : (
                <button key={link.href} onClick={() => scrollToSection(link.href)} className="text-slate-800 hover:text-green-600 transition-colors relative group font-medium cursor-pointer">
                  {link.label}
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#10B981] group-hover:w-full transition-all"></span>
                </button>
              )
